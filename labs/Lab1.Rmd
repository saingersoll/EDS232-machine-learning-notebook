...
new_pumpkins <- pumpkins %>%                                       # data frame called
  select(variety, city_name, package, price, date, month, day) %>% # columns of interest to keep
  filter(
    str_detect(package, 'bushel'))                                 # remove row observations in that do not contain the phrase 'bushel' in the package column
```
...
corr_mat <- cor(baked_pumpkins) %>% 
  select(-c(low_price, high_price))                    # Drop columns that are not really informative
