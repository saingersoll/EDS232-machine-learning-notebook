...
```{r}
# deploys a field of trees to determine the best
?rand_forest
```
...
# takes too long so load data instead
...
```{r}
test_predict_rf = predict(train_fit_rf, test) %>% #get prediction for test 
  bind_cols(test) %>%  #bind to testing column
  mutate(habitat = as.factor(habitat))

test_predict2_rf = predict(train_fit_rf, test, type = "prob") %>% #get testing prediction probabilities
  bind_cols(test) %>%  #bind to testing column
  mutate(habitat = as.factor(habitat))
```

```{r}
test_predict_xgb = predict(train_fit_xgb, test) %>% #get prediction probabilities for test 
  bind_cols(test) %>%  #bind to testing column
  mutate(habitat = as.factor(habitat))

test_predict2_xgb = predict(train_fit_xgb, test, type = "prob") %>% #get testing prediction
  bind_cols(test) %>%  #bind to testing column
  mutate(habitat = as.factor(habitat))
```
