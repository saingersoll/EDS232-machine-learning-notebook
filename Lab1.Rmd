...
ggplot(pumpkins) +
  geom_point(aes(x = day,
                 ...
                 color = month),
             alpha = 0.5) +
  scale_color_gradient(low = '#FCAE1E', high = '#C95B0C') +
  labs(title = "Price of Pumpkins Throughout the Year (2016-2017)",
       x = 'Day of the Year',
       y = 'Price of Pumpinks')
```
...
*Question 3:* Is this a useful plot? Does anything about it surprise you? How do we make it more useful? To get charts to display useful data, you usually need to group the data somehow.

This plot was slightly informative. It informs us of the price of pumpkins vs the days of the year and the points are color coded according to month, but it is not easy to quickly to differentiate trends using the information displayed. To no surpise, the cost of pumpkins is higher in months that pumpkins are not in season, and cheapest during their harvesting season (fall months). To improve this, I would make the x-axis by month rather than day and convert the size of the points to match the amount of pumpkins sold during that month.
...
monthly_pumpkin_prices <- new_pumpkins %>% 
  group_by(month) %>% 
  summarize(price = mean(price))

monthly_pumpkin_prices

ggplot(monthly_pumpkin_prices, aes(x = month,
                                   y = price)) +
  geom_col(fill ='#CC5801',
           alpha = 0.7) +
  ylab('Fall Months Pumpkin Prices')
```
...
pumpkins %>%
  ...
  geom_col(fill = "#CC5801", alpha = 0.7) +
  ylab("Pumpkin Price")
```
...
cor(baked_pumpkins$price, baked_pumpkins$month)

cor(baked_pumpkins$price, baked_pumpkins$variety)
```

*Question 7:* Which of these three variables is most highly correlated with price? Why might this be?

The pumpkin package type is the most highly correlated with price. This is likely due to the ability to buy in bulk to lower the price. 
...
```{r prediction_test, warning = FALSE}
...
A major distribution of the data displays high variance from the linear regression line. This is not the most accurate fit of the data. This data is likely best represented by a polynomial function, we will explore more later. :-)
